...
function atualizarStatusDrive() {
    ...
    if (btnAgenda) {
        if (conectado) {
            ...
            btnAgenda.style.color = '#e87fa0';
            btnAgenda.title = '⚠️ Clique para salvar seus agendamentos no Google Drive';
        }
    }
...
function tokenValido() {
    // Aceita token salvo mesmo após nosso timer — a API avisa com 401 se realmente expirou
    const token = S.googleToken || lsGet('agenda_google_token', null);
    if (!token) return false;
    if (!S.googleToken) S.googleToken = token; // restaura da memória se perdido
    const exp = lsGet('agenda_google_token_exp', 0);
    // Considera válido se dentro do prazo OU se nunca foi definido (migração)
        ...
        try {
            ...
            if (hash) {
                ...
                if (token) {
                    ...
                    lsSet('agenda_google_token_exp', Date.now() + 86400 * 1000);
